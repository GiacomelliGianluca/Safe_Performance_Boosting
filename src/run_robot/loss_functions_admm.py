...
class RobotsLoss():
    ...
    def forward(self, xs, us, xsp = None, usp= None, lambda_sx = None, lambda_su = None, for_plt = False):
        """
        Compute loss.
        ...
        if xsp is not None:
            xp_batch = xsp.reshape(*xsp.shape, 1)
            up_batch = usp.reshape(*usp.shape, 1)
            lambda_x_batch = lambda_sx.reshape(*lambda_sx.shape, 1)
            lambda_u_batch = lambda_su.reshape(*lambda_su.shape, 1)
            # Projection Loss ADMM
            loss_constr = self.loss_proj_admm(x_batch, u_batch, xp_batch, up_batch, lambda_x_batch, lambda_u_batch)
            if not for_plt:
    ...
    def loss_proj_admm(self, x_batch, u_batch, xp_batch, up_batch, lambda_x_batch, lambda_u_batch):

        loss_proj = 0

        if self.rho  > 0:
            x_arg = x_batch - xp_batch + lambda_x_batch
            x_diff_norm = torch.matmul(x_arg.transpose(-1, -2), x_arg)
            loss_proj_x = torch.sum(x_diff_norm, 1)

            u_arg = u_batch - up_batch + lambda_u_batch
            u_diff_norm = torch.matmul(u_arg.transpose(-1, -2), u_arg)
            loss_proj_u = torch.sum(u_diff_norm, 1)

            loss_proj = self.rho/2*(loss_proj_x + loss_proj_u)
